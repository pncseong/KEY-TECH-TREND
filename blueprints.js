...
window.EngineeringBlueprints = {
    dynamicSvgs: {},
    registerDynamicSvg: function(nodeId, svgCode) {
        if (!this.dynamicSvgs) this.dynamicSvgs = {};
        this.dynamicSvgs[nodeId] = svgCode;
    },
    getSvg: function(nodeId) {
        if (this.dynamicSvgs && this.dynamicSvgs[nodeId]) {
            return this.dynamicSvgs[nodeId];
        }
        switch(nodeId) {
            case 'hbm4_foundry':
                return getHbm4Svg();
...
// 13. 유리 코어 기판 (Glass Core Substrate w/ TGV) 정밀 엔지니어링 단면도
